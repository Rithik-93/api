...
export const swiggyHandler = async (req: Request, res: Response) => {
  try {
    ...
    const sourceSheet = sourceWorkbook.Sheets["Sheet1"];
    if (!sourceSheet) {
      return res
        .status(400)
        .json({ error: "Sheet1 sheet not found" });
    }

    const sourceData = xlsx.utils.sheet_to_json(sourceSheet, { header: 1 });
    const sourceHeaderRow = sourceData[1];
    //@ts-ignore
    const billAmountIndex = sourceHeaderRow.findIndex((val) =>
      val
        ?.toString()
        .toLowerCase()
        .includes("order total (inclusive of gst) (a)")
    );
    //@ts-ignore
    ...
    if (billAmountIndex === -1 || sourceDateIndex === -1) {
      return res.status(400).json({
        error: "Missing 'order total (inclusive of gst) (a)' or 'Date and time' in source file",
      });
    }
